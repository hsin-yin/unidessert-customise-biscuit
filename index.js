...
app.get('/product/productInfo', function (req, res) {
    ...
    conn.query(`select * from user where uemail='${req.session.user.email}'`, (err, results) => {
        ...
        conn.query(`select * from orderlist where uid=? && order_status="購物車"`, [uid], (err, results) => {
            ...
            // 當會員之前沒有加過購物車
            if (!results[0]) {
                conn.query(`INSERT INTO orderlist (oid, uid, deliever_fee, order_total, order_date, recipient, recipient_address, recipient_phone, recipient_email, arrive_date, payment_type, order_status) VALUES (NULL, ?, 150, ?, '${formattedDate}', "", "", "", "", '${formattedDate}', "", "購物車")`,
                    [uid, order_total],
                    (err, results) => {
                        if (err) {
                            console.log(err.message);
